...
const stamp = '20260514T0134Z'
const id = `stream-2-secure-vps-house-v3-${stamp}`
...
const copy = {
  title: 'Secure VPS, No Leaks',
  hook: 'The first build-in-public rule: ask AI for infrastructure help, but explicitly protect secrets before the screen goes live.',
  caption: 'Masala starts the VPS setup by giving the AI the right constraint: help me build it live, but make sure nothing private leaks on stream.',
  youtubeTitle: 'Secure VPS Setup Live: The No-Leaks Rule',
  fileName: 'secure-vps-setup-live-no-leaks-rule-short.mp4',
  tiktokCaption: 'First rule of building with AI live: get help, but protect secrets before anything hits the stream. #buildinpublic #vibecoding #openclaw #aitools',
  thumbnailText: 'SECURE VPS / NO LEAKS',
}
...
const vf = [
  ...
  `[underlay]drawtext=fontfile=${fontBold}:text='${esc('SECURE VPS')}':x=(w-text_w)/2:y=86:fontcolor=white:fontsize=88:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('NO LEAKS')}':x=(w-text_w)/2:y=178:fontcolor=white:fontsize=94:borderw=5:bordercolor=black[headline]`,
  ...
  `[withscreen][logo]overlay=x=(W-w)/2:y=1082:format=auto,drawtext=fontfile=${fontBold}:text='${esc('VIBE ZONE')}':x=(w-text_w)/2:y=1190:fontcolor=white@0.98:fontsize=62:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('BUILD LIVE, PROTECT SECRETS')}':x=(w-text_w)/2:y=1264:fontcolor=white@0.96:fontsize=56:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('AI HELP + STREAM SAFETY')}':x=(w-text_w)/2:y=1336:fontcolor=white@0.94:fontsize=52:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('HOUSE STYLE V3')}':x=(w-text_w)/2:y=1406:fontcolor=white@0.84:fontsize=42:borderw=5:bordercolor=black[branded]`,
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '9', '-i', out, '-frames:v', '1', '-vf', "scale=1080:1920,drawbox=x=44:y=72:w=992:h=252:color=black@0.32:t=fill", '-q:v', '2', `${exportDir}/thumbnail-secure-vps-no-leaks.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-secure-vps-no-leaks-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-secure-vps-no-leaks-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-secure-vps-no-leaks.jpg`), path.join(root, `${readyDir}/short-secure-vps-no-leaks-house-v3-thumbnail.jpg`))

const metadata = {
  id: `clip_stream2_secure_vps_house_v3_${stamp}`,
  title: copy.title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-secure-vps-no-leaks-house-v3.mp4`,
  thumbnailPath: `${readyDir}/short-secure-vps-no-leaks-house-v3-thumbnail.jpg`,
  hook: copy.hook,
  caption: copy.caption,
  hashtags: ['#BuildInPublic', '#VibeCoding', '#OpenClaw', '#AITools', '#CreatorOps'],
  seo: {
    primaryKeyword: 'secure VPS setup live stream',
    searchIntent: 'Creators and indie builders who want safer live AI-assisted infrastructure setup without leaking secrets.',
    ...
    tags: ['secure VPS setup', 'build in public', 'AI coding livestream', 'OpenClaw', 'vibe coding', 'creator operations', 'livestream safety'],
    titleVariants: ['The No-Leaks Rule For Building Live', 'Secure VPS Setup Without Leaking Secrets', 'Build Live, Protect Secrets'],
    pinnedComment: 'Would you let an AI help with infrastructure live if it had strict no-leak rules first?',
  ...
  createdAt: '2026-05-14T01:34:00.000Z',
}
...
const clip = {
  platform: 'youtube+tiktok-manual',
  status: 'exported',
  exportedAt: '2026-05-14T01:34:00.000Z',
  id: clipId,
  ...
  reason: 'Style-corrected Stream 2 VPS/no-leaks moment into current house-v3 layout; supersedes the older facecam-smart secure VPS package pending owner privacy pass.',
  createdAt: metadata.createdAt,
  sourceIdeaId: `stream2_secure_vps_house_v3_${stamp}`,
  videoId: 'stream-2',
  renderStatus: 'done',
