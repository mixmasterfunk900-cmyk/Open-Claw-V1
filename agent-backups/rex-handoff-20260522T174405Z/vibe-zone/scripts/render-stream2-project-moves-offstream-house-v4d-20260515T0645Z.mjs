...
const readyDir = `media/exports/READY_TO_SHIP_NOW/stream2-project-moves-offstream-house-v4d-${stamp}`
const exportDir = `media/exports/stream2-project-moves-offstream-house-v4d-${stamp}`

const clip = {
  id: 'clip_stream2_project_moves_offstream_house_v4d_20260515T0645Z',
...
const id = `stream-2-project-moves-offstream-house-v4d-${stamp}`
...
const readyPath = `${readyDir}/short-project-moves-offstream-house-v4d.mp4`
...
const vf = [
  ...
  `[srcmain]crop=iw*0.34:ih-92:0:92,scale=${videoW}:${videoH}:force_original_aspect_ratio=increase,crop=${videoW}:${videoH},setsar=1[screenfit]`,
...
const manifest = {
  ...
  seo,
  reviewNotes: [
    'Fresh Stream 2 local-only improvement render using Confirmed Template #1 retention border + travelling shine, centered-screen house v4 layout, and VIBE ZONE + OpenClaw branding.',
    '0.5s seed frame prepended for manual Shorts thumbnail-selection tests.',
    'White one-word captions stay in the safe zone above the brand lane; source screen is tight active-left-zoomed to reduce dead desktop space.',
...
const clipRecord = {
  ...
  renderPath: out,
  renderUrl: `/${out}`,
  renderPreset: 'stream2-house-v4d-confirmed-template-retention-border-shine-seed',
  renderStatus: 'done',
