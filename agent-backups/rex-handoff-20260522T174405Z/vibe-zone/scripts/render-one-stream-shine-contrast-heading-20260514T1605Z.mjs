...
const id = 'day3-one-stream-shine-contrast-heading-20260514T1605Z'
...
const wordTimingPath = 'media/transcripts/stream3-top10-word-timings-20260514T1035Z/01-day3-one-stream-many-clips.words.json'
...
const startSec = 15 * 60 + 12
const endSec = 15 * 60 + 32
...
const frameW = 1016
const frameH = 614
const captionMarginV = 560 // down a tiny bit from 600
...
const wordTimedEvents = (wordTiming.words || []).map((word) => ({ start: word.start, end: word.end, text: cleanCaptionWord(word.text) })).filter((word) => word.text && word.text !== 'AI')
...
chains.push(`[underlay]drawtext=fontfile='${fontBold}':text='${esc('STREAM ONCE')}':x=(w-text_w)/2:y=${headerZoneTop}+34:fontcolor=0xFFD400:fontsize=128:borderw=7:bordercolor=black@0.82[head1]`)
chains.push(`[head1]drawtext=fontfile='${fontBold}':text='${esc('FOREVER')}':x=(w-text_w)/2:y=${headerZoneTop}+164:fontcolor=0xFFD400:fontsize=142:borderw=7:bordercolor=black@0.82[headline]`)
...
for (const start of [0.7, 6.2, 12.4, 17.0]) {
...
run('ffmpeg', ['-y', '-v', 'error', '-i', source, '-filter_complex', filterComplex, '-map', '[vout]', '-map', '[aout]', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '160k', '-shortest', out])
...
await writeFile(path.join(root, exportDir, 'notes.md'), `# Wrapped heading + lower captions + shine test\n\nChanges: heading wraps as STREAM ONCE / FOREVER, title uses yellow/black contrast for light backgrounds, captions moved slightly down (ASS MarginV ${captionMarginV}), thin green frame retained, real travelling shine retained. Discussion/test render only.\n`)
...
console.log(JSON.stringify({ id, out, ready, review, exportDir, captionQa, ffprobe: JSON.parse(probe.stdout) }, null, 2))
