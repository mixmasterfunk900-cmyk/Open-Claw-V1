...
const startSec = 158
const duration = 22
const id = 'stream-2-build-clip-machine-centered-logo-fix-20260513T1916Z'
...
// Fixed owner-feedback layout:
// - centered source video in a balanced screen panel
// - title moved lower, no longer hugging the top edge
// - plain VIBE ZONE text removed
// - actual logo asset used as the brand mark
const vf = [
  'color=c=0x050505:s=1080x1920:r=30:d=22[base]',
  '[0:v]scale=992:-2:force_original_aspect_ratio=decrease,setsar=1[screenfit]',
  '[1:v]scale=174:-1,format=rgba[logo]',
  '[base][logo]overlay=x=(W-w)/2:y=72:format=auto[branded]',
  `[branded]drawtext=fontfile=${fontBold}:text='${esc('BUILD THE')}':x=(w-text_w)/2:y=230:fontcolor=white:fontsize=76:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('CLIP MACHINE')}':x=(w-text_w)/2:y=310:fontcolor=white:fontsize=82:borderw=5:bordercolor=black[headline]`,
  '[headline]drawbox=x=44:y=430:w=992:h=700:color=black@0.62:t=fill,drawbox=x=44:y=430:w=992:h=700:color=white@0.24:t=4[panel]',
...
await writeFile(path.join(root, bundleDir, 'review-notes.md'), `# Review Notes — Build Clip Machine centered/logo fix\n\nFixes Masala feedback from 2026-05-13:\n\n- Centered the source video in the screen panel.\n- Moved “BUILD THE / CLIP MACHINE” down from the top edge.\n- Removed plain “VIBE ZONE” text.\n- Added actual logo asset at the top.\n- Kept one-word short captions and passed caption QA.\n\nManual upload only. Owner visual approval still required.\n`)
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-build-the-clip-machine-centered-logo-fix.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-centered-logo-fix-contact-sheet.jpg'))
console.log(JSON.stringify({ id, out, ready: `${readyDir}/short-build-the-clip-machine-centered-logo-fix.mp4`, assPath, captionQa: qa, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, ffprobe: JSON.parse(probe.stdout) }, null, 2))
