...
const stamp = '20260514T0800Z'
const id = `template-trial-no-leaks-${stamp}`
...
const captionAssColor = '&H0000FF66' // neon green (ASS is AABBGGRR)
...
let wordTimedEvents = []
try {
  const wordTiming = JSON.parse(await readFile(path.join(root, `media/transcripts/${id}.words.json`), 'utf8'))
  wordTimedEvents = normalizeTiming((wordTiming.words || []).map((word) => ({ start: word.start, end: word.end, text: word.text })), { minDuration: 0.14, maxDuration: 0.75, gap: 0.015, videoDuration: duration })
} catch {
  wordTimedEvents = []
}
const events = wordTimedEvents.length ? wordTimedEvents : fallbackEvents
...
const vf = [
  ...
  `[underlay]drawtext=fontfile='${fontBold}':text='${esc(headerText)}':x=(w-text_w)/2:y=${headerZoneTop}+(${headerZoneBottom}-${headerZoneTop}-text_h)/2:fontcolor=${headerColor}:fontsize=164:borderw=5:bordercolor=black@0.70[headline]`,
  `[srcmain]crop=iw:ih-92:0:92,scale=${videoW}:${videoH}:force_original_aspect_ratio=decrease,pad=${videoW}:${videoH}:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1[screenfit]`,
  `[headline]drawbox=x=${videoX}:y=${videoY}:w=${videoW}:h=${videoH}:color=black@0.42:t=fill,drawbox=x=${videoX}:y=${videoY}:w=${videoW}:h=${videoH}:color=white@0.70:t=4[panel]`,
  `[panel][screenfit]overlay=x=${videoX}:y=${videoY}:format=auto[withscreen]`,
  // Fixed invisible logo zone: reserved every render, filled only when a relevant external brand applies.
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '12', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/thumbnail-template-trial.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-template-trial-no-leaks.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-template-trial-no-leaks-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-template-trial.jpg`), path.join(root, readyDir, 'short-template-trial-no-leaks-thumbnail.jpg'))
console.log(JSON.stringify({ id, out, readyCopy: `${readyDir}/short-template-trial-no-leaks.mp4`, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, captionQa: qa, ffprobe: JSON.parse(probe.stdout) }, null, 2))
