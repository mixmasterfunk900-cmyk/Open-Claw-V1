...
const readyDir = `media/exports/READY_TO_SHIP_NOW/stream2-project-moves-offstream-house-v4c-${stamp}`
const exportDir = `media/exports/stream2-project-moves-offstream-house-v4c-${stamp}`

const clip = {
  id: 'clip_stream2_project_moves_offstream_house_v4c_20260515T0635Z',
  title: 'Project Moves Off-Stream',
  hook: `BUILD WHILE
OFFSTREAM`,
  start: 192.0,
  end: 212.0,
...
const id = `stream-2-project-moves-offstream-house-v4c-${stamp}`
...
const readyPath = `${readyDir}/short-project-moves-offstream-house-v4c.mp4`
...
let captionEvents = buildShortWordEvents({
  ...
  maxDuration: 0.68,
  gap: 0.012,
}).map((event) => ({ ...event, text: cleanCaptionWord(event.text) })).filter((event) => event.text && !['UM', 'UH'].includes(event.text) && !(event.text === 'I' && event.end - event.start < 0.3))
...
const vf = [
  ...
  `[srcmain]crop=iw*0.52:ih-92:0:92,scale=${videoW}:${videoH}:force_original_aspect_ratio=increase,crop=${videoW}:${videoH},setsar=1[screenfit]`,
...
const manifest = {
  ...
  seo,
  reviewNotes: [
    'Fresh Stream 2 local-only improvement render using the centered-screen house v4 layout with VIBE ZONE + OpenClaw branding.',
    '0.5s seed frame prepended for manual Shorts thumbnail-selection tests.',
    'White one-word captions stay in the safe zone above the brand lane; source screen is tighter-left-zoomed to reduce dead desktop space.',
...
const clipRecord = {
  ...
  hashtags: clip.hashtags,
  reason: 'Fresh Stream 2 improvement render: concise off-stream progress thesis, rendered with current house-style centered-screen layout and seed frame.',
  createdAt: now,
  renderPath: out,
  renderUrl: `/${out}`,
  renderPreset: 'stream2-house-v4c-active-left-zoom-safe-header-seed-offstream-progress',
  renderStatus: 'done',
