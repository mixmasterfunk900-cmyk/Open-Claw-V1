...
const readyDir = `media/exports/READY_TO_SHIP_NOW/stream2-clips-first-vps-next-house-v4b-${stamp}`
const exportDir = `media/exports/stream2-clips-first-vps-next-house-v4b-${stamp}`

const clip = {
  id: 'clip_stream2_clips_first_vps_next_house_v4b_20260515T0650Z',
  title: 'Clips First, VPS Next',
  hook: 'CLIPS FIRST VPS NEXT',
  start: 135.0,
  end: 179.0,
  score: 87,
  caption: 'The founder roadmap in one clean Stream 2 moment: set up social accounts, start posting stream clips, then wire the VPS so the build can keep moving.',
  hashtags: ['#VibeZone', '#BuildInPublic', '#StreamerTools', '#CreatorTools', '#AI'],
}
...
const id = `stream-2-clips-first-vps-next-house-v4b-${stamp}`
...
const readyPath = `${readyDir}/short-clips-first-vps-next-house-v4b.mp4`
...
const seo = {
  youtubeTitle: 'The Stream Plan: Clips First, VPS Next',
  youtubeDescription: `A clean build-in-public roadmap from Stream 2: set up the social accounts, start posting stream clips, then wire up a VPS so the Vibe Zone build can keep moving even off-stream.\n\nPrepared as a local manual-upload bundle only. No external posting, cookies, logins, or API writes were used.`,
  tiktokDescription: 'The roadmap was simple: social accounts, stream clips, then a VPS so the build can keep moving. #VibeZone #BuildInPublic #CreatorTools #AI',
  hashtags: clip.hashtags,
  suggestedPinnedComment: 'Would you set up clips first or automation infrastructure first?',
  thumbnailText: 'CLIPS FIRST VPS NEXT',
}
...
const manifest = {
  ...
  seo,
  reviewNotes: [
    'Fresh Stream 2 local-only render using Confirmed Template #1 retention border + travelling shine, centered-screen house v4 layout, and VIBE ZONE + OpenClaw branding.',
    ...
    'Stream-safe roadmap clip: social accounts, clips, and VPS planning only; no secrets/logins/cookies/API posting.',
  ],
}
...
const clipRecord = {
  ...
  hashtags: clip.hashtags,
  reason: 'Fresh Stream 2 improved overnight production clip: concise roadmap from social setup to clips to VPS, rendered with the current house-style layout.',
  createdAt: now,
  renderPath: out,
  renderUrl: `/${out}`,
  renderPreset: 'stream2-house-v4b-confirmed-template-retention-border-shine-seed',
  renderStatus: 'done',
