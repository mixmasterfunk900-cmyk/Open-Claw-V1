...
const duration = 28
const id = 'stream-2-ai-agents-real-work-house-style-v3-20260513T1942Z'
...
const exportDir = `media/exports/clip_stream2_ai_agents_real_work_house_style_v3_20260513T1942Z`
...
// House style v2: keep the centered source screen primary, but add a dim blurred
// source underlay so the lower half is not wasted black space. No blue card.
const vf = [
  '[0:v]split=2[srcmain][srcbg]',
  '[srcbg]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,boxblur=18:1,eq=brightness=-0.10:saturation=0.70[bg]',
  `color=c=0x050505:s=1080x1920:r=30:d=${duration}[base]`,
  '[base][bg]overlay=0:0:format=auto,drawbox=x=0:y=0:w=1080:h=300:color=black@0.70:t=fill,drawbox=x=0:y=1030:w=1080:h=890:color=black@0.08:t=fill[underlay]',
  '[srcmain]scale=992:-2:force_original_aspect_ratio=decrease,setsar=1[screenfit]',
  '[1:v]scale=220:-1,format=rgba[logo]',
  `[underlay]drawtext=fontfile=${fontBold}:text='${esc('AI AGENTS')}':x=(w-text_w)/2:y=92:fontcolor=white:fontsize=86:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('DO REAL WORK')}':x=(w-text_w)/2:y=184:fontcolor=white:fontsize=78:borderw=5:bordercolor=black[headline]`,
  ...
  '[withscreen][logo]overlay=x=(W-w)/2:y=1080:format=auto,drawtext=fontfile=${fontBold}:text=\'LOCAL-FIRST\':x=(w-text_w)/2:y=1240:fontcolor=white@0.96:fontsize=76:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text=\'STREAM-SAFE\':x=(w-text_w)/2:y=1325:fontcolor=white@0.94:fontsize=68:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text=\'SHIPS OFFLINE\':x=(w-text_w)/2:y=1405:fontcolor=white@0.94:fontsize=68:borderw=5:bordercolor=black[branded]',
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-ai-agents-real-work-house-style-v3.mp4'))
...
await writeFile(path.join(root, postReadyDir, 'review-notes.md'), `# Review Notes — AI Agents Do Real Work house-style v3 rerender\n\n- Replaces the 1934Z/1938Z attempts, which still wasted the lower half, and suspended/older blue-card/square-face variants with current house style.\n- Title above, centered source screen, logo below video, one-word captions.\n- No external posting/API/cookie/login work.\n- Needs final human privacy spot-check because source screen context is visible.\n`)
console.log(JSON.stringify({ id, out, exportDir, readyCopy: `${readyDir}/short-ai-agents-real-work-house-style-v3.mp4`, assPath, captionQa: qa, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, ffprobe: JSON.parse(probe.stdout) }, null, 2))
