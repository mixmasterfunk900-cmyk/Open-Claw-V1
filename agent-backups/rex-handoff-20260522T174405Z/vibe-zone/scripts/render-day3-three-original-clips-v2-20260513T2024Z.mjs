...
const stamp = '20260513T2024Z'
const reviewDir = 'media/reviews/day3-originals'
const readyDir = 'media/exports/READY_TO_SHIP_NOW/day3-originals-v2'
...
for (const spec of clips) {
  ...
  const vf = [
    ...
    `[withscreen]drawbox=x=(w-480)/2:y=${spec.logoY}:w=480:h=164:color=${spec.accent}@0.18:t=fill,drawbox=x=(w-480)/2:y=${spec.logoY}:w=480:h=164:color=white@0.32:t=4,drawtext=fontfile=${fontBold}:text='${esc(spec.brand)}':x=(w-text_w)/2:y=${spec.logoY + 22}:fontcolor=white:fontsize=80:borderw=5:bordercolor=black,drawtext=fontfile=${fontSans}:text='${esc(spec.brandSub)}':x=(w-text_w)/2:y=${spec.logoY + 106}:fontcolor=white:fontsize=24:borderw=3:bordercolor=black[branded]`,
