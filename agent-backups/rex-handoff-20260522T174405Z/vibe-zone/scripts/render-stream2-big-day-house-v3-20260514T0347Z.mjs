...
const stamp = '20260514T0347Z'
const id = `stream-2-big-day-sprint-house-v3-${stamp}`
...
const copy = {
  title: 'Big Day Build Sprint',
  hook: 'Big day energy: clear plan, live build, and the clip machine starts moving.',
  caption: 'Masala opens the stream with the simple mission: build in public, wire up the creator workflow, and turn one focused session into reusable Vibe Zone clips.',
  youtubeTitle: 'Big Day Build Sprint | Vibe Zone Build In Public',
  fileName: 'big-day-build-sprint-vibe-zone-short.mp4',
  tiktokCaption: 'Big day energy: build in public, clip the process, and keep the machine moving. #buildinpublic #vibecoding #openclaw #creatorops',
  thumbnailText: 'BIG DAY BUILD SPRINT',
}
...
const vf = [
  ...
  `[underlay]drawtext=fontfile=${fontBold}:text='${esc('BIG DAY')}':x=(w-text_w)/2:y=86:fontcolor=white:fontsize=94:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('BUILD SPRINT')}':x=(w-text_w)/2:y=178:fontcolor=white:fontsize=88:borderw=5:bordercolor=black[headline]`,
  ...
  `[withscreen][logo]overlay=x=(W-w)/2:y=1082:format=auto,drawtext=fontfile=${fontBold}:text='${esc('VIBE ZONE')}':x=(w-text_w)/2:y=1190:fontcolor=white@0.98:fontsize=62:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('STREAM ONCE, CLIP FOREVER')}':x=(w-text_w)/2:y=1264:fontcolor=white@0.96:fontsize=54:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('BUILD IN PUBLIC MOMENT')}':x=(w-text_w)/2:y=1336:fontcolor=white@0.94:fontsize=52:borderw=5:bordercolor=black[branded]`,
...
for (const [name, ss] of [['proof-frame-01.jpg', '2'], ['proof-frame-02.jpg', '10'], ['proof-frame-03.jpg', '20']]) {
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '10', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/thumbnail-big-day-build-sprint.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-big-day-build-sprint-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-big-day-build-sprint-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-big-day-build-sprint.jpg`), path.join(root, `${readyDir}/short-big-day-build-sprint-house-v3-thumbnail.jpg`))

const metadata = {
  id: `clip_stream2_big_day_sprint_house_v3_${stamp}`,
  title: copy.title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-big-day-build-sprint-house-v3.mp4`,
  thumbnailPath: `${readyDir}/short-big-day-build-sprint-house-v3-thumbnail.jpg`,
  hook: copy.hook,
  caption: copy.caption,
  hashtags: ['#BuildInPublic', '#VibeCoding', '#OpenClaw', '#CreatorOps', '#Shorts'],
  seo: {
    primaryKeyword: 'build in public sprint',
    searchIntent: 'Creators and builders looking for energetic build-in-public workflow examples.',
    youtubeTitle: copy.youtubeTitle,
    fileName: copy.fileName,
    description: `${copy.caption}\n\nLocal manual-upload draft only. Owner approval and a final privacy watch pass are required before public posting.\n\n#BuildInPublic #VibeCoding #OpenClaw #CreatorOps #Shorts`,
    tiktokCaption: copy.tiktokCaption,
    tags: ['build in public', 'creator workflow', 'Vibe Zone', 'OpenClaw', 'vibe coding', 'clip machine'],
    titleVariants: ['Big Day Build Sprint', 'Stream Once, Clip Forever', 'The Vibe Zone Build Starts'],
    pinnedComment: 'Would you rather watch the polished launch or the messy build sprint?',
  ...
  createdAt: '2026-05-14T03:47:00.000Z',
}
...
const clip = {
  ...
  transcriptId: 'tx_1778603901735_e1b588', score: 84, start: metadata.start, end: metadata.end,
  title: metadata.title, hook: metadata.hook, caption: metadata.caption, hashtags: metadata.hashtags,
  reason: 'Style-corrected Stream 2 cold-open sprint into current house-v3 layout; supersedes legacy centered-screen package pending owner privacy pass.',
  createdAt: metadata.createdAt, sourceIdeaId: `stream2_big_day_sprint_house_v3_${stamp}`, videoId: 'stream-2',
