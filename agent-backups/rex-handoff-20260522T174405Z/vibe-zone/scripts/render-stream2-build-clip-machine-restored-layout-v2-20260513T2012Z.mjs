...
const startSec = 158
const duration = 22
const id = 'stream-2-build-clip-machine-restored-layout-v2-20260513T2012Z'
...
const vf = [
  ...
  '[withscreen][logo]overlay=x=(W-w)/2:y=1450:format=auto[branded]',
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-build-the-clip-machine-restored-layout-v2.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-restored-layout-v2-contact-sheet.jpg'))
console.log(JSON.stringify({ id, out, ready: `${readyDir}/short-build-the-clip-machine-restored-layout-v2.mp4`, assPath, captionQa: qa, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, ffprobe: JSON.parse(probe.stdout) }, null, 2))
