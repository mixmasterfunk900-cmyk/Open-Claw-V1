...
type PageId = 'roadmap' | 'dashboard' | 'youtube-automation' | 'media-pipeline' | 'clip-factory' | 'thumbnail-lab' | 'viral-research' | 'viral-hunter' | 'studio-feedback' | 'social-dashboard' | 'twitter-radar' | 'live-chat' | 'settings'
...
const navItems = [
  ...
  { id: 'youtube-automation', label: 'YouTube Automation', icon: '🐾', kicker: 'Channel pipelines' },
...
function Page({ page, state, refresh, setError, liveChat }: { page: PageId; state: AppState; refresh: () => Promise<void>; setError: (value: string) => void; liveChat: LiveChatController }) {
  ...
  switch (page) {
    ...
    case 'youtube-automation': return <YouTubeAutomation state={state} />
...
function YouTubeAutomation({ state }: { state: AppState }) {
  ...
  const isScript = (file: MediaFile) => /\.(md|txt|json|srt|vtt)$/i.test(file.name) || /transcript|script|source-notes|visual-plan|manifest|captions/i.test(file.name)
  const cleanAssetLabel = (name: string) => name
    .replace(/\.(mp4|mov|webm|mkv|png|jpe?g|md|txt|json|srt|vtt)$/i, '')
    .replace(/the[-_ ]experiment[-_ ]where[-_ ]/gi, '')
    .replace(/ordinary[-_ ]people[-_ ]obeyed[-_ ]a[-_ ]stranger/gi, 'Ordinary People Obeyed A Stranger')
    .replace(/children[-_ ]were[-_ ]taught[-_ ]to[-_ ]fear/gi, 'Children Were Taught To Fear')
    .replace(/frame[-_ ]generation[-_ ]manifest/gi, 'Frame Generation Manifest')
    .replace(/visual[-_ ]plan[-_ ]draft/gi, 'Visual Plan Draft')
    .replace(/script[-_ ]v1[-_ ]voiceover[-_ ]only/gi, 'Voiceover Script')
    .replace(/script[-_ ]v1/gi, 'Full Script')
    .replace(/source[-_ ]notes/gi, 'Source Notes')
    .replace(/final[-_ ]audio[-_ ]transcript/gi, 'Final Audio Transcript')
    .replace(/final[-_ ]audio[-_ ]captions/gi, 'Final Audio Captions')
    .replace(/[-_]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  const assetKindLabel = (file: MediaFile) => {
    if (/manifest/i.test(file.name)) return 'Manifest'
    if (/visual[_ -]?plan/i.test(file.name) || file.path.includes('/visual-plan/')) return 'Plan'
    if (/source[_ -]?notes/i.test(file.name)) return 'Research'
    if (/caption|\.srt$|\.vtt$/i.test(file.name)) return 'Captions'
    if (/voiceover|script|transcript/i.test(file.name)) return 'Script'
    if (isVideo(file)) return 'Video'
    if (isImage(file)) return 'Image'
    return 'File'
  }
  const assetIcon = (file: MediaFile) => ({ Manifest: '▦', Plan: '⌘', Research: '◎', Script: '¶', Captions: '≋', Video: '▶', Image: '◇', File: '•' }[assetKindLabel(file)] || '•')
  ...
  const lanes: YouTubeLane[] = [
    ...
    },
    {
      ...
      projectOrder: ['the-experiment-where-ordinary-people-obeyed-a-stranger', 'the-experiment-where-children-were-taught-to-fear'],
      projectMeta: {
        'the-experiment-where-ordinary-people-obeyed-a-stranger': { title: 'The Experiment Where Ordinary People Obeyed A Stranger', status: 'image generation draft', description: 'Newest overnight Experiment Where project: Milgram-style obedience script, source notes, visual plan, contact sheets, and 71 generated draft frames.' },
    ...
    },
    {
      ...
      shortLabel: 'Finance',
      icon: '💸',
      rootMatch: 'youtube-automation/finance-content',
      status: 'review-ready',
      emptyIcon: '💸',
      description: 'Bespoke money/consumer-finance YouTube cluster with its own SOP, scripts, visuals, thumbnails, captions, and QA gates.',
      plannedDetail: 'Finance explainers / money psychology / grounded product-comparison style',
      projectOrder: ['credit-card-minimum-payment-trap'],
      projectMeta: {
        'credit-card-minimum-payment-trap': { title: 'Credit Card Minimum Payment Trap', status: 'complete review', description: 'Full finance-lane review package: burned-caption browser copy, clean master, thumbnail, subtitles, script, manifest, and QA reports.' },
      },
    },
  ]
  ...
  const selectedVideo = selectedBuckets.videos.find((file) => /site-visible-captions.*\.mp4$/i.test(file.path)) ?? selectedBuckets.videos.find((file) => /burned[-_](captions|subs).*\.mp4$/i.test(file.path)) ?? selectedBuckets.videos.find((file) => /preview-1min.*visible-captions\.mp4$/i.test(file.path)) ?? selectedBuckets.videos.find((file) => /audio-truth-captioned-v2\.mp4$|audio-truth-captioned\.mp4$|practice-v3-one-line-audio-subtitles\.mp4$|practice-v2-netflix-subtitles\.mp4$|practice-v1\.mp4$/i.test(file.path)) ?? selectedBuckets.videos[0]
  ...
  const pipelines = laneSummaries.map(({ lane, assets }) => {
    ...
    const hasReviewVideo = assets.some((file) => isVideo(file) && /audio-truth-captioned|preview-1min|practice-v|burned[-_](captions|subs)|clean[-_]master/i.test(file.name))
  ...
  const assetLinks = (files: MediaFile[], empty: string) => files.length ? <div className="youtube-asset-links">{files.slice(0, 40).map((file) => <a className="youtube-asset-pill" href={mediaUrl(file.url, file.updatedAt)} target="_blank" download key={file.path} title={file.name}><em aria-hidden="true">{assetIcon(file)}</em><span><strong>{cleanAssetLabel(file.name)}</strong><small>{assetKindLabel(file)} • {new Date(file.updatedAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}</small></span><b>{formatBytes(file.size)}</b></a>)}</div> : <p>{empty}</p>
