...
function MediaPipeline({ state, refresh, setError }: { state: AppState; refresh: () => Promise<void>; setError: (value: string) => void }) {
  const latestStream = state.videos.find((video) => /\blive\b|stream|vibe coding|day \d+/i.test(video.title)) || state.videos[0]
