...
function MediaPipeline({ state, refresh, setError }: { state: AppState; refresh: () => Promise<void>; setError: (value: string) => void }) {
  ...
  const conceptTitle = (file: MediaFile) => file.name
    ...
    .replace(/\s+/g, ' ')
    .trim()
  const conceptTag = (file: MediaFile) => file.path.includes('stream4-template-v2') ? 'Stream 4 V2' : file.path.includes('contextual-short') ? 'Template V2' : file.path.includes('script-aware-short') ? 'Script-aware test' : 'Concept'
