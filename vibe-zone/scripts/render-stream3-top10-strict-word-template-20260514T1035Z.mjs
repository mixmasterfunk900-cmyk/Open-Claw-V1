...
const exportBase = `media/exports/stream3-top10-strict-word-template-${stamp}`
const readyDir = 'media/exports/READY_TO_SHIP_NOW/day3-top10-strict-word-template'
...
await writeFile(path.join(root, exportBase, 'manifest.json'), JSON.stringify({ createdAt: new Date().toISOString(), source, transcriptPath, candidatePath, template: 'permanent-fixed-template-strict-word-timing', results }, null, 2) + '\n')
await writeFile(path.join(root, readyDir, 'README.md'), `# Day 3 top 10 fixed-template shorts\n\nRendered ${results.length} clips with the approved permanent Vibe Zone short template and fresh exact-cut Whisper word-level timings.\n\n${results.map((r) => `${r.rank}. ${r.headerText} — ${r.readyPath}`).join('\n')}\n`)
