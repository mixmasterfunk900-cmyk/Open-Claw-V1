...
const id = 'day3-one-stream-shine-wrapped-heading-20260514T1600Z'
...
chains.push(`[underlay]drawtext=fontfile='${fontBold}':text='${esc('STREAM ONCE')}':x=(w-text_w)/2:y=${headerZoneTop}+34:fontcolor=white:fontsize=128:borderw=5:bordercolor=black@0.70[head1]`)
chains.push(`[head1]drawtext=fontfile='${fontBold}':text='${esc('FOREVER')}':x=(w-text_w)/2:y=${headerZoneTop}+164:fontcolor=white:fontsize=142:borderw=5:bordercolor=black@0.70[headline]`)
...
await writeFile(path.join(root, exportDir, 'notes.md'), `# Wrapped heading + lower captions + shine test\n\nChanges: heading wraps as STREAM ONCE / FOREVER, captions moved slightly down (ASS MarginV ${captionMarginV}), thin green frame retained, real travelling shine retained. Discussion/test render only.\n`)
