...
const stamp = '20260514T0852Z'
const id = `thumbnail-looks-mid-template-${stamp}`
...
const headerText = 'THUMBNAIL MID'
const headerColor = 'white'
const captionAssColor = '&H0000FF66' // neon green, different from header (ASS is AABBGGRR)
...
const vf = [
  ...
  // Fixed invisible logo zone: reserved every render, filled only when a relevant external brand applies. None applies here.
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '9', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/thumbnail-template.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-thumbnail-looks-mid-template.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-thumbnail-looks-mid-template-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-template.jpg`), path.join(root, readyDir, 'short-thumbnail-looks-mid-template-thumbnail.jpg'))
console.log(JSON.stringify({ id, out, readyCopy: `${readyDir}/short-thumbnail-looks-mid-template.mp4`, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, captionQa: { ...qa, timingSource: captionPlan.source, coverage: captionPlan.coverage, primaryCoverage: captionPlan.primaryCoverage }, ffprobe: JSON.parse(probe.stdout), zones: { headerZoneTop, headerZoneBottom, videoX, videoY, videoW, videoH, logoZoneTop, logoZoneH, captionMarginV } }, null, 2))
