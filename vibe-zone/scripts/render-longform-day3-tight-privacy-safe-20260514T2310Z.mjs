...
const id = 'day3-product-content-machine-tight-privacy-safe-20260514T2310Z'
...
const beats = [
  ...
  { order: 3, role: 'blocker', title: 'The rough part', start: '00:24:00.4', end: '00:24:38.4', chapter: '00:56', excerpt: 'The functionality works, but the brain behind it all is just terrible.', rationale: 'Creates honest tension; the machine exists but quality is not there yet.' },
  ...
  { order: 6, role: 'result', title: 'The agent loop', start: '00:44:48.4', end: '00:45:48.4', chapter: '02:30', excerpt: 'Planner agent, film agent, work agents... when I am off stream.', rationale: 'Pays off the system idea: the build can keep moving while Masala is away.' },
  { order: 7, role: 'cta', title: 'Next bottleneck: face tracking', start: '00:47:13.4', end: '00:47:46.4', chapter: '03:30', excerpt: 'The face tracking / facecam problem becomes the next concrete build hook.', rationale: 'Ends with a specific next episode, not a vague wrap-up.' },
]
...
const story = {
  ...
  notes: 'Tighter privacy-safe Day 3 upload-candidate refresh. Full livestream image is blurred/dimmed before captions/logo. No external posting/logins/cookies/public exposure.'
}
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'long-day3-product-content-machine-tight-privacy-safe-20260514T2310Z.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'long-day3-product-content-machine-tight-privacy-safe-contact-sheet-20260514T2310Z.jpg'))
await copyFile(path.join(root, assPath.replace(/\.ass$/, '.qa.json')), path.join(root, readyDir, 'long-day3-product-content-machine-tight-privacy-safe-caption-qa-20260514T2310Z.json'))

console.log(JSON.stringify({ id, out, ready: `${readyDir}/long-day3-product-content-machine-tight-privacy-safe-20260514T2310Z.mp4`, duration, captions: assPath, captionQa: qa, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, ffprobe: JSON.parse(probe.stdout) }, null, 2))
