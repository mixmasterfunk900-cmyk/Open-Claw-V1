...
const stamp = '20260514T0452Z'
const id = `stream-2-social-to-vps-plan-house-v3-privacycrop-${stamp}`
...
const vf = [
  ...
  `[underlay]drawtext=fontfile='${fontBold}':text='SOCIALS TO\nVPS PLAN':x=(w-text_w)/2:y=76:fontcolor=white:fontsize=92:line_spacing=-8:borderw=3:bordercolor=black@0.70[headline]`,
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-socials-to-vps-plan-house-v3-privacycrop.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-socials-to-vps-plan-house-v3-privacycrop-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-socials-to-vps.jpg`), path.join(root, `${readyDir}/short-socials-to-vps-plan-house-v3-privacycrop-thumbnail.jpg`))

const metadata = {
  id: `clip_stream2_social_to_vps_plan_house_v3_privacycrop_${stamp}`,
  title: copy.title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-socials-to-vps-plan-house-v3-privacycrop.mp4`,
  thumbnailPath: `${readyDir}/short-socials-to-vps-plan-house-v3-privacycrop-thumbnail.jpg`,
  hook: copy.hook,
...
const clip = {
  ...
  createdAt: metadata.createdAt, sourceIdeaId: `stream2_social_to_vps_plan_house_v3_privacycrop_${stamp}`, videoId: 'stream-2',
