...
const stamp = '20260514T1208Z'
const id = `stream-2-ai-still-working-house-v3-${stamp}`
...
const startSec = 4865.36
const endSec = 4898.0
const duration = endSec - startSec

const headerText = 'AI STILL WORKING'
...
const captionEvents = captionPlan.events.filter((event) => event.text !== 'AI')
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-ai-still-working-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-ai-still-working-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail.jpg`), path.join(root, readyDir, 'short-ai-still-working-house-v3-thumbnail.jpg'))
for (const [i, t] of [4, 9, 15].entries()) {
...
const uploadCard = `# Upload Card — AI Still Working

- Render: ${out}
- Ready copy: ${readyDir}/short-ai-still-working-house-v3.mp4
- Source: Stream 2, 81:05–81:38
- Template: Restored house-style v3; centered/cropped source-screen context, OpenClaw logo + VIBE ZONE branding, clean white hook/captions, no blue-card artifact, no square-face default.
- Hook: ${headerText}
- Story angle: The funny founder moment where the agent keeps working while Rex can finally just watch/smile — exactly the autonomous-build promise.
...
- Privacy: final owner watch pass required because source-screen text is visible; no external posting performed.
`
await writeFile(path.join(root, exportDir, 'upload-card.md'), uploadCard)

const youtube = `# YouTube Shorts Draft — AI Still Working

Title: AI Still Working While I Smile

Description:
The best part of building in public: the agent keeps moving while I just sit there smiling. This is the Vibe Zone loop — stream the build, let OpenClaw turn the work into content, then improve the machine overnight.

Hashtags:
#BuildInPublic #AIAgents #OpenClaw #VibeCoding #StartupJourney #YouTubeShorts

Pinned comment idea:
Would you trust an AI agent to keep building while you sleep?

Manual upload notes:
- Upload manually only after owner privacy/watch pass.
- Use thumbnail.jpg or proof-frame-02.jpg; both keep the source-screen context and house branding.
`
await writeFile(path.join(root, exportDir, 'youtube-upload.md'), youtube)

const tiktok = `# TikTok Draft — AI Still Working

Caption:
When the AI agent is still working and you realize the content machine might actually work 😅

On-screen hook:
AI STILL WORKING

Hashtags:
#aibuild #vibecoding #buildinpublic #startup #aitools #openclaw

Manual upload notes:
- Manual upload only; no external posting/API writes performed.
- Owner privacy/watch pass required because source-screen UI is visible.
`
await writeFile(path.join(root, exportDir, 'tiktok-upload.md'), tiktok)
console.log(JSON.stringify({ id, out, readyCopy: `${readyDir}/short-ai-still-working-house-v3.mp4`, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, captionQa: { ...qa, timingSource: captionPlan.source, coverage: captionPlan.coverage, primaryCoverage: captionPlan.primaryCoverage }, ffprobe: JSON.parse(probe.stdout), zones: { headerZoneTop, headerZoneBottom, videoX, videoY, videoW, videoH, logoZoneTop, logoZoneH, captionMarginV } }, null, 2))
