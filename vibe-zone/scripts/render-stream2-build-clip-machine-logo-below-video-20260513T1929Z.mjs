...
const startSec = 158
const duration = 22
const id = 'stream-2-build-clip-machine-logo-below-video-20260513T1929Z'
...
// Owner layout: title above, centered video, logo BELOW video.
const vf = [
  ...
  `[base]drawtext=fontfile=${fontBold}:text='${esc('BUILD THE')}':x=(w-text_w)/2:y=92:fontcolor=white:fontsize=78:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('CLIP MACHINE')}':x=(w-text_w)/2:y=174:fontcolor=white:fontsize=84:borderw=5:bordercolor=black[headline]`,
...
await writeFile(path.join(root, bundleDir, 'review-notes.md'), `# Review Notes — logo below video fix\n\nFixes Masala feedback:\n- Title remains above.\n- Video remains centered.\n- Logo is now below the video, not below the title.\n- Plain VIBE ZONE text remains removed.\n- One-word captions preserved and QA-passed.\n`)
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-build-the-clip-machine-logo-below-video.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-logo-below-video-contact-sheet.jpg'))
console.log(JSON.stringify({ id, out, ready: `${readyDir}/short-build-the-clip-machine-logo-below-video.mp4`, assPath, captionQa: qa, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, ffprobe: JSON.parse(probe.stdout) }, null, 2))
