...
const duration = 28
const id = 'stream-2-ai-agents-real-work-house-style-v2-20260513T1938Z'
...
const exportDir = `media/exports/clip_stream2_ai_agents_real_work_house_style_v2_20260513T1938Z`
...
const vf = [
  ...
  '[srcbg]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,boxblur=18:1,eq=brightness=-0.28:saturation=0.55[bg]',
  `color=c=0x050505:s=1080x1920:r=30:d=${duration}[base]`,
  '[base][bg]overlay=0:0:format=auto,drawbox=x=0:y=0:w=1080:h=300:color=black@0.70:t=fill,drawbox=x=0:y=1030:w=1080:h=890:color=black@0.30:t=fill[underlay]',
  ...
  '[withscreen][logo]overlay=x=(W-w)/2:y=1074:format=auto,drawtext=fontfile=${fontBold}:text=\'LOCAL-FIRST  •  STREAM-SAFE  •  SHIPS OFFLINE\':x=(w-text_w)/2:y=1238:fontcolor=white@0.92:fontsize=42:borderw=4:bordercolor=black[branded]',
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-ai-agents-real-work-house-style-v2.mp4'))
...
await writeFile(path.join(root, postReadyDir, 'review-notes.md'), `# Review Notes — AI Agents Do Real Work house-style v2 rerender\n\n- Replaces the 1934Z attempt, which still wasted the lower half, and suspended/older blue-card/square-face variants with current house style.\n- Title above, centered source screen, logo below video, one-word captions.\n- No external posting/API/cookie/login work.\n- Needs final human privacy spot-check because source screen context is visible.\n`)
console.log(JSON.stringify({ id, out, exportDir, readyCopy: `${readyDir}/short-ai-agents-real-work-house-style-v2.mp4`, assPath, captionQa: qa, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, ffprobe: JSON.parse(probe.stdout) }, null, 2))
