...
const id = `socials-to-vps-template-${stamp}`
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '9', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/socials-to-vps-template-thumbnail.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-socials-to-vps-template.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-socials-to-vps-template-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/socials-to-vps-template-thumbnail.jpg`), path.join(root, readyDir, 'short-socials-to-vps-template-thumbnail.jpg'))
console.log(JSON.stringify({ id, out, readyCopy: `${readyDir}/short-socials-to-vps-template.mp4`, contactSheet: `${reviewDir}/${id}-contact-sheet.jpg`, captionQa: { ...qa, timingSource: captionPlan.source, coverage: captionPlan.coverage, primaryCoverage: captionPlan.primaryCoverage }, ffprobe: JSON.parse(probe.stdout), zones: { headerZoneTop, headerZoneBottom, videoX, videoY, videoW, videoH, logoZoneTop, logoZoneH, captionMarginV } }, null, 2))
