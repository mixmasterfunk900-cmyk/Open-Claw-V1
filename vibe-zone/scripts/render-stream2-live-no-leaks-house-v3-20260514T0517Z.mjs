...
const stamp = '20260514T0517Z'
const id = `stream-2-live-no-leaks-house-v3-${stamp}`
...
const copy = {
  title: 'The First Rule Is No Leaks',
  hook: 'Before the build gets fast, the stream has one rule: do not leak anything.',
  caption: 'A live build only works if the setup is safe first: hide private details, check the screen, then ship in public.',
  youtubeTitle: 'The First Rule Is No Leaks | Vibe Zone',
  fileName: 'first-rule-no-leaks-vibe-zone-short.mp4',
  tiktokCaption: 'Live build rule one: move fast, but do not leak anything. #buildinpublic #opsec #vibecoding #openclaw',
...
const vf = [
  ...
  `[underlay]drawtext=fontfile='${fontBold}':text='FIRST RULE\nNO LEAKS':x=(w-text_w)/2:y=76:fontcolor=white:fontsize=92:line_spacing=-8:borderw=3:bordercolor=black@0.70[headline]`,
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '12', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/thumbnail-no-leaks.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-first-rule-no-leaks-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-first-rule-no-leaks-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-no-leaks.jpg`), path.join(root, `${readyDir}/short-first-rule-no-leaks-house-v3-thumbnail.jpg`))

const metadata = {
  id: `clip_stream2_live_no_leaks_house_v3_${stamp}`,
  title: copy.title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-first-rule-no-leaks-house-v3.mp4`,
  thumbnailPath: `${readyDir}/short-first-rule-no-leaks-house-v3-thumbnail.jpg`,
  hook: copy.hook,
  caption: copy.caption,
  hashtags: ['#BuildInPublic', '#OpSec', '#VibeCoding', '#OpenClaw', '#Shorts'],
  seo: {
    primaryKeyword: 'build in public opsec',
    searchIntent: 'Creators learning how to stream live builds without leaking private information.',
    ...
    tags: ['build in public', 'opsec', 'live coding', 'Vibe Zone', 'OpenClaw', 'creator workflow'],
    titleVariants: ['The First Rule Is No Leaks', 'Screen Check Before Shipping', 'Live Build OpSec Rule'],
    pinnedComment: 'What do you always hide before a live build?',
  ...
  createdAt: '2026-05-14T05:17:00.000Z',
}
