...
const stamp = '20260514T0817Z'
const id = `stream-2-billion-company-no-experience-house-v3-${stamp}`
...
const copy = {
  title: 'Billion Dollar Company, No Experience',
  hook: 'The ridiculous thesis is the whole point: build the company from scratch while learning in public.',
  caption: 'A raw build-in-public moment from Stream 2: no polished founder myth, just the ambition and the messy first reps.',
  youtubeTitle: 'Billion Dollar Company, No Experience | Vibe Zone Build In Public',
  fileName: 'billion-dollar-company-no-experience-vibe-zone-short.mp4',
  tiktokCaption: 'The thesis is ridiculous on purpose: build the company from scratch while learning in public. #buildinpublic #startupjourney #vibecoding #openclaw',
  thumbnailText: 'NO EXPERIENCE',
}
...
const vf = [
  ...
  `[underlay]drawtext=fontfile=${fontBold}:text='${esc('BILLION-DOLLAR')}':x=(w-text_w)/2:y=82:fontcolor=white:fontsize=88:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('COMPANY?')}':x=(w-text_w)/2:y=174:fontcolor=white:fontsize=104:borderw=5:bordercolor=black[headline]`,
  ...
  `[withscreen][logo]overlay=x=(W-w)/2:y=1082:format=auto,drawtext=fontfile=${fontBold}:text='${esc('VIBE ZONE')}':x=(w-text_w)/2:y=1190:fontcolor=white@0.98:fontsize=62:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('STREAM ONCE, CLIP FOREVER')}':x=(w-text_w)/2:y=1264:fontcolor=white@0.96:fontsize=54:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('LEARN IN PUBLIC')}':x=(w-text_w)/2:y=1336:fontcolor=white@0.94:fontsize=50:borderw=5:bordercolor=black[branded]`,
...
for (const [name, ss] of [['proof-frame-01.jpg', '3'], ['proof-frame-02.jpg', '12'], ['proof-frame-03.jpg', '21']]) {
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '12', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/thumbnail-billion-company-no-experience.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-billion-company-no-experience-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-billion-company-no-experience-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-billion-company-no-experience.jpg`), path.join(root, `${readyDir}/short-billion-company-no-experience-house-v3-thumbnail.jpg`))

const metadata = {
  id: `clip_stream2_billion_company_no_experience_house_v3_${stamp}`,
  title: copy.title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-billion-company-no-experience-house-v3.mp4`,
  thumbnailPath: `${readyDir}/short-billion-company-no-experience-house-v3-thumbnail.jpg`,
  hook: copy.hook,
  caption: copy.caption,
  hashtags: ['#BuildInPublic', '#StartupJourney', '#VibeCoding', '#OpenClaw', '#Shorts'],
  seo: {
    primaryKeyword: 'build in public startup journey',
    searchIntent: 'Creators following a raw founder/build-in-public AI workflow journey.',
    youtubeTitle: copy.youtubeTitle,
    fileName: copy.fileName,
    description: `${copy.caption}\n\nLocal manual-upload draft only. Owner approval and a final privacy watch pass are required before public posting.\n\n#BuildInPublic #StartupJourney #VibeCoding #OpenClaw #Shorts`,
    tiktokCaption: copy.tiktokCaption,
    tags: ['build in public', 'startup journey', 'AI agents', 'Vibe Zone', 'OpenClaw', 'vibe coding', 'founder story'],
    titleVariants: ['Billion Dollar Company, No Experience', 'Build The Company From Scratch', 'The Ridiculous Founder Thesis'],
    pinnedComment: 'Would you rather watch the polished win or the messy first reps?',
  ...
  createdAt: '2026-05-14T08:17:00.000Z',
}
...
const clip = {
  ...
  transcriptId: 'tx_1778603901735_e1b588', score: 87, start: metadata.start, end: metadata.end,
  title: metadata.title, hook: metadata.hook, caption: metadata.caption, hashtags: metadata.hashtags,
  reason: 'Style-corrected Stream 2 founder-thesis moment into current house-v3 layout; supersedes legacy billion-company package pending owner privacy pass.',
  createdAt: metadata.createdAt, sourceIdeaId: `stream2_billion_company_no_experience_house_v3_${stamp}`, videoId: 'stream-2',
