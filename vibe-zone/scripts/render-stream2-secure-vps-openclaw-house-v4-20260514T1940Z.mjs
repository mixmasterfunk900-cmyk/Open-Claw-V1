...
const readyDir = `media/exports/READY_TO_SHIP_NOW/stream2-secure-vps-openclaw-house-v4-${stamp}`
const exportDir = `media/exports/clip_stream2-secure-vps-openclaw-house-v4-${stamp}`

const clip = {
  id: 'clip_stream2_secure_vps_openclaw_20260514T1940Z',
  title: 'Secure VPS For The AI Build',
  hook: 'SECURE THE AI VPS',
  start: 436.0,
  end: 450.0,
  score: 95,
  caption: 'A tight Stream 2 moment where the live build rule is clear: keep the stream on, but make the VPS secure and never leak sensitive setup details.',
  hashtags: ['#VibeZone', '#BuildInPublic', '#AI', '#OpenClaw'],
}
...
const id = `stream-2-secure-vps-openclaw-house-v4-${stamp}`
...
const readyPath = `${readyDir}/short-secure-vps-openclaw-house-v4.mp4`
...
const manifest = {
  ...
  seo,
  reviewNotes: [
    'Fresh Stream 2 local-only render using the current centered-screen house layout on a security/setup moment.',
    ...
    'Source section is stream-safe in sampled proof frames: it discusses security goals without exposing readable secrets; final owner watch/privacy pass still required.',
  ],
}
...
await writeFile(path.join(root, exportDir, 'tiktok-upload.md'), `# TikTok Draft\n\nCaption: ${seo.tiktokDescription}\n\nManual upload only. Do not post externally without explicit approval.\n`)
await writeFile(path.join(root, exportDir, 'upload-card.md'), `# Upload Card — ${clip.title}\n\n- Local render: ${out}\n- Ready copy: ${readyPath}\n- Thumbnail: ${thumbnail}\n- Proof: ${contactSheet}\n- YouTube draft: youtube-upload.md\n- TikTok draft: tiktok-upload.md\n- Status: local manual-review bundle; external posting blocked pending approval.\n`)
...
const clipRecord = {
  ...
  hashtags: clip.hashtags,
  reason: 'Fresh Stream 2 overnight production clip: secure VPS/OpenClaw setup moment with stream-safe local-only packaging.',
  createdAt: new Date().toISOString(),
  renderPath: out,
  renderUrl: `/${out}`,
  renderPreset: 'stream2-house-v4-centered-screen-security-moment',
  renderStatus: 'done',
...
const dispatchRecord = {
  ...
  clipId: clip.id,
  title: clip.title,
  status: 'approved_manual_upload_after_owner_privacy_pass',
  ...
  proofFramePath: contactSheet,
  createdAt: new Date().toISOString(),
  note: 'Prepared local-only upload bundle; do not externally post without explicit Masala approval and owner privacy/watch pass.',
}
