...
const stamp = '20260514T0647Z'
const id = `stream-2-black-screen-flop-house-v3-${stamp}`
...
const copy = {
  title: 'My Test Clips Flopped',
  hook: 'The first automated clips did not fail quietly — they came out as a black screen.',
  caption: 'Masala catches the exact kind of build-in-public failure Vibe Zone is meant to fix: the clip tests ran, but the output was a black screen.',
  youtubeTitle: 'My Test Clips Flopped | Vibe Zone Build In Public',
  fileName: 'my-test-clips-flopped-vibe-zone-short.mp4',
  tiktokCaption: 'The first automated clips came out as a black screen. That is the build-in-public moment worth fixing live. #buildinpublic #vibecoding #creatorops #openclaw',
  thumbnailText: 'CLIPS FLOPPED',
}
...
const vf = [
  ...
  // Crop the browser top bar out before fitting the screen context. Square face box stays out of the default layout.
  ...
  `[underlay]drawtext=fontfile=${fontBold}:text='${esc('TEST CLIPS')}':x=(w-text_w)/2:y=86:fontcolor=white:fontsize=94:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('FLOPPED')}':x=(w-text_w)/2:y=178:fontcolor=white:fontsize=98:borderw=5:bordercolor=black[headline]`,
  ...
  `[withscreen][logo]overlay=x=(W-w)/2:y=1082:format=auto,drawtext=fontfile=${fontBold}:text='${esc('VIBE ZONE')}':x=(w-text_w)/2:y=1190:fontcolor=white@0.98:fontsize=62:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('STREAM ONCE, CLIP FOREVER')}':x=(w-text_w)/2:y=1264:fontcolor=white@0.96:fontsize=54:borderw=5:bordercolor=black,drawtext=fontfile=${fontBold}:text='${esc('BUILD-IN-PUBLIC FAILURE')}':x=(w-text_w)/2:y=1336:fontcolor=white@0.94:fontsize=50:borderw=5:bordercolor=black[branded]`,
...
for (const [name, ss] of [['proof-frame-01.jpg', '2'], ['proof-frame-02.jpg', '10'], ['proof-frame-03.jpg', '18']]) {
...
run('ffmpeg', ['-y', '-v', 'error', '-ss', '10', '-i', out, '-frames:v', '1', '-vf', 'scale=1080:1920', '-q:v', '2', `${exportDir}/thumbnail-test-clips-flopped.jpg`])
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-test-clips-flopped-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-test-clips-flopped-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail-test-clips-flopped.jpg`), path.join(root, `${readyDir}/short-test-clips-flopped-house-v3-thumbnail.jpg`))

const metadata = {
  id: `clip_stream2_black_screen_flop_house_v3_${stamp}`,
  title: copy.title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-test-clips-flopped-house-v3.mp4`,
  thumbnailPath: `${readyDir}/short-test-clips-flopped-house-v3-thumbnail.jpg`,
  hook: copy.hook,
  caption: copy.caption,
  hashtags: ['#BuildInPublic', '#VibeCoding', '#CreatorOps', '#OpenClaw', '#Shorts'],
  seo: {
    primaryKeyword: 'build in public clip workflow',
    searchIntent: 'Creators looking for honest behind-the-scenes AI clip workflow failures and fixes.',
    ...
    tags: ['build in public', 'creator workflow', 'AI clips', 'Vibe Zone', 'OpenClaw', 'vibe coding'],
    titleVariants: ['My Test Clips Flopped', 'The Clip Tests Came Out Black', 'Build In Public Means Showing Failed Clips'],
    pinnedComment: 'Would you show the failed clip tests or only the polished result?',
  ...
  createdAt: '2026-05-14T06:47:00.000Z',
}
...
const clip = {
  ...
  reason: 'Style-corrected Stream 2 black-screen/flopped-tests moment into current house-v3 layout; supersedes legacy centered-screen package pending owner privacy pass.',
  createdAt: metadata.createdAt, sourceIdeaId: `stream2_black_screen_flop_house_v3_${stamp}`, videoId: 'stream-2',
