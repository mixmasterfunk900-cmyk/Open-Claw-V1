...
const stamp = '20260514T1510Z'
const id = `stream-2-phone-controls-build-house-v3-${stamp}`
...
const wordTimingPath = 'media/transcripts/stream-2-phone-controls-build-house-v3-20260514T1510Z.words.json'
...
const title = 'Control The Build From My Phone'
const headerText = 'BUILD RUNS\nFROM MY PHONE'
...
for (const [i, t] of [3, 8, 13].entries()) {
...
await copyFile(path.join(root, out), path.join(root, readyDir, 'short-control-build-from-phone-house-v3.mp4'))
await copyFile(path.join(root, `${reviewDir}/${id}-contact-sheet.jpg`), path.join(root, readyDir, 'short-control-build-from-phone-house-v3-contact-sheet.jpg'))
await copyFile(path.join(root, `${exportDir}/thumbnail.jpg`), path.join(root, readyDir, 'short-control-build-from-phone-house-v3-thumbnail.jpg'))

const youtubeCopy = `# YouTube Shorts Upload Draft\n\nTitle: Control The Build From My Phone | Offline Agent Workflow\n\nDescription:\nThe stream moment where the build stops being tied to the desk: set up the VPS, connect OpenClaw, and control progress from a phone while off-stream.\n\nLocal manual-upload draft only. Owner approval and a final privacy watch pass are required before public posting.\n\nHashtags: #BuildInPublic #AgentWorkflow #VibeCoding #OpenClaw #YouTubeShorts\n\nPinned comment idea: Would you run your build from your phone if the agent handled the boring parts?\n`
const tiktokCopy = `# TikTok Upload Draft\n\nCaption: The build starts moving when the workflow runs even after the stream ends: VPS + OpenClaw + phone control. Manual upload only after owner privacy/watch pass. #buildinpublic #agentworkflow #vibecoding #openclaw\n`
const uploadCard = `# Upload Card — ${title}\n\n- Render: ${out}\n- Ready copy: ${readyDir}/short-control-build-from-phone-house-v3.mp4\n- Source: Stream 2, ${startSec}s–${endSec}s\n- Improves/safe replacement for: legacy Stream 2 stop-overbuilding-and-ship-the-workflow facecam/card exports\n- Template: Restored house-style v3; centered/cropped source screen, blurred source underlay, OpenClaw logo + VIBE ZONE branding, clean white hook/captions, no blue card, no square-face default.\n- Hook: ${headerText.replace('\\n', ' / ')}\n- Thumbnail: ${exportDir}/thumbnail.jpg\n- Contact sheet: ${reviewDir}/${id}-contact-sheet.jpg\n- Proof frames: ${exportDir}/proof-frame-01.jpg, proof-frame-02.jpg, proof-frame-03.jpg\n- Caption QA: ${captionEvents.length} one-word events, pass; timing source ${captionPlan.source}.\n- SEO: offline agent workflow / control build from phone / OpenClaw VPS setup.\n- Privacy: final owner watch pass required because source screen/UI text is visible; no external posting performed.\n`
const metadata = {
  id: `clip_stream2_phone_controls_build_house_v3_${stamp}`,
  title,
  ...
  renderPath: out,
  readyCopyPath: `${readyDir}/short-control-build-from-phone-house-v3.mp4`,
  thumbnailPath: `${readyDir}/short-control-build-from-phone-house-v3-thumbnail.jpg`,
  hook: 'The goal is simple: make the build move from your phone when you are off-stream.',
  caption: 'A Stream 2 workflow moment: VPS plus OpenClaw turns the live build into something you can keep steering from your phone after the stream.',
  hashtags: ['#BuildInPublic', '#AgentWorkflow', '#VibeCoding', '#OpenClaw', '#YouTubeShorts'],
  seo: {
    primaryKeyword: 'offline agent workflow',
    searchIntent: 'Creators learning how agents and VPS workflows keep projects moving off-stream.',
    youtubeTitle: 'Control The Build From My Phone | Offline Agent Workflow',
    fileName: 'control-build-from-phone-vibe-zone-short.mp4',
    description: youtubeCopy.split('Description:\n')[1].split('\n\nHashtags:')[0],
    tiktokCaption: tiktokCopy.split('Caption: ')[1].trim(),
    tags: ['build in public', 'agent workflow', 'VPS setup', 'vibe coding', 'OpenClaw', 'creator workflow'],
    titleVariants: ['Control The Build From My Phone', 'Build From Anywhere', 'The Off-Stream Agent Workflow'],
    pinnedComment: 'Would you run your build from your phone if the agent handled the boring parts?',
    thumbnailText: 'PHONE CONTROL',
  },
